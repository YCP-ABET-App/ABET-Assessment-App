...
class PerformanceIndicatorRepositoryTest {
     ...
     */
    private PerformanceIndicator newIndicator(String description, int num, long soId, long programId) {
    ...
    void shouldFindByDescriptionContainingIgnoreCase() {
        newIndicator("Ability to COMMUNICATE effectively", 1, 1L, 1L);
        newIndicator("Students must COMMUNICATE clearly", 2, 1L, 1L);
        newIndicator("Unrelated skill", 3, 1L, 1L); // Should NOT match
    ...
    void shouldFindByStudentOutcomeId() {
        newIndicator("Outcome 1 - A", 1, 1L, 1L);
        newIndicator("Outcome 1 - B", 2, 1L, 1L);
        newIndicator("Outcome 2 - A", 3, 2L, 1L);
    ...
    void shouldFindActiveIndicatorsByStudentOutcomeId() {
        PerformanceIndicator active1 = newIndicator("Active 1", 1, 1L, 1L);
        PerformanceIndicator active2 = newIndicator("Active 2", 2, 1L, 1L);

        PerformanceIndicator inactive = newIndicator("Inactive", 3, 1L, 1L);
    ...
    void shouldDetectExistingIndicatorNumber() {
        newIndicator("Test Indicator", 5, 2L, 1L);
